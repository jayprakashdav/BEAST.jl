...
function laplacekernel(op::FiniteDiffConvolutionQuadrature, s, T)
	...
	error("laplacekernel not implemented for $(typeof(op.timedomainkernel))")
end
...
function assemble(cqop :: FiniteDiffConvolutionQuadrature,
	...
	if threading.parameters[1]==:single
		@debug "single threaded assembly"
		for q = 0:Qmax-1
	...
	if threading.parameters[1]==:single
		@debug "single threaded inv z transform"
		for q = 0:kmax-1
			Z[:,:,q+1] = real_inverse_z_transform(q, rho, Q, Zz)
		end
	elseif threading.parameters[1]==:multi
		@debug "multi threaded inv z transform"
		T = Threads.nthreads()
		# split over the full 0:kmax-1 range (kmax = Q); splitting up to Qmax
		# only (as the assembly loop does for the conjugate-symmetric Zz) would
		# leave the time slices beyond Qmax zero
