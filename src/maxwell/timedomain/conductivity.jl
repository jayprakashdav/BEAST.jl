...
# keyed on generic RefSpace (not LinearRefSpaceTriangle) so that GWP div/curl
# bases dispatch here as well; the Space-Space method is needed because
# localop.jl resolves quadstrat with Space arguments and the LocalOperator
# Space-Space fallback would win otherwise
BEAST.defaultquadstrat(::BEAST.ConductivityTD_Functionaltype, ::BEAST.RefSpace, ::BEAST.RefSpace) = BEAST.SingleNumQStrat(10)
BEAST.defaultquadstrat(::BEAST.ConductivityTD_Operatortype, ::BEAST.RefSpace, ::BEAST.RefSpace) = BEAST.SingleNumQStrat(10)
BEAST.defaultquadstrat(::BEAST.ConductivityTD_Operatortype, ::BEAST.Space, ::BEAST.Space) = BEAST.SingleNumQStrat(10)
BEAST.defaultquadstrat(::BEAST.ConductivityTD_Functionaltype, ::BEAST.Space) = BEAST.SingleNumQStrat(10)

function quaddata(exc::BEAST.ConductivityTD_Functionaltype, g::BEAST.RefSpace, f::BEAST.RefSpace, tels, bels,
...
function celltestvalues(tshs::BEAST.RefSpace{T}, t, tcell, field::ConductivityTDFunc, qr) where {T}
    ...
    # the global RHS vector is allocated with scalartype(field, tfs) (real for
    # real efield data); a Complex local here would InexactError on store
