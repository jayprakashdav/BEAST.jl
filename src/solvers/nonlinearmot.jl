function update!(op::ConductivityTD_Functionaltype, jcoeffs, ecoeffs, k, W1, V1; quadstrat=defaultquadstrat(op, W1.space, V1.space))
    tgeo = W1.space.geo
    bgeo = V1.space.geo
    #= V1 = nothing
    W1 = nothing
    ...
    if CompScienceMeshes.refines(tgeo, bgeo)
        op.efield = quadpoint_field_refines(op, ecoeffs, k, W1, V1)
        #op.jflux = quadpoint_field_refines(op, jcoeffs, k, W2, V1)
    else
        op.efield = quadpoint_field(op, ecoeffs, k,  W1, V1;quadstrat=quadstrat)
...
function quadpoint_field_refines(biop::ConductivityTD_Functionaltype, coeffs, k, tfs, bfs;
        ...
        numbrefs = BEAST.numfunctions(brefs, bdom)
...
function marchonintimenl(eq1, eq2,  Z, inc, Ġ, G_j, G_nl, Nt)
    ...
    # e is expanded in eq2's trial space and tested with eq2's test space
    ...
    for i in 1:Nt
        ...
        for l in 1:iter_max
            if l==1
                ...
                xeprev = xe[:,i-1]
                update!(σ, xj, xe, i-1, W2, V2)
            else
                xeprev = xe[:,i]
                update!(σ, xj, xe, i, W2, V2)
            end
            ...
            if maximum(xe[:,i]-xeprev) < 1e-6
                break
            end
            println("maximum diff xe ", maximum(xe[:,i]-xeprev))
...
# The augmented-EFIE solver (td_solve_augmented/marchonintimeaug) is not
# ported to this branch; see nlmodelling2 for the archived implementation.
